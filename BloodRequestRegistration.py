...
# SHOW INFORMATION
def helpmessage():
    message = messagebox.showinfo("HELP","information of Blood bank")
...
def store():
    ...
    with open('RequestorRegistration.csv', 'a',newline='') as csvfile:
        ...
        writer.writerow([NIC_store,name_store, age_store, dob_store, bloodgrp_store, phone_store, city_store, state_store])
        csvfile.close()
...
heading = Label(text="Requestor Registration",font=("verdana",30,"bold"),fg="Yellow",bg="magenta").place(x=480,y=40)
...
## NIC NUMBER INPUT
NICno = Label(text="NIC\nNumber",width="12",bg="blue",fg="white").place(x=677,y=305)
NICinput = Entry(textvariable=NICno,width=16)
NICinput.place(x=775,y=310)
...
###ALL BUTTTON HERE
submitbutton = Button(root,text="SUBMIT",width="20",font=("verdana",12,"bold"),fg="red",bg="yellow",command=store)
submitbutton.place(x=633,y=500)

helptbutton = Button(root,text="HELP",width="8",font=("verdana",12,"bold"),fg="Red",bg="white",command=helpmessage)
helptbutton.place(x=645,y=550)

exittbutton = Button(root,text="EXIT",width="8",font=("verdana",12,"bold"),fg="Red",bg="white",command=exitprgrm)
exittbutton.place(x=747,y=550)
